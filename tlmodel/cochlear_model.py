...
libtrisolv.solve_tridiagonal.argtypes = [
    ctypes.POINTER(tridiag_matrix),  # aa
    PDOUBLE,  # vv
    PDOUBLE,  # solution
    INT,  # nrows
]
...
libtrisolv.delay_line.argtypes = [
    PDOUBLE,  # in_matrix
    PINT,  # delay1
    PINT,  # delay2
    PINT,  # delay1
...
def TLsolver(t, y, model):  # y''=dv/dt y'=v
    ...
    if model.non_linearity:  # non-linearities here
        ...
        # larger than 1%
        if np.max(abs(SheraP[1:n] - model.SheraP[1:n]) / abs(model.SheraP[1:n])) > 0.01:
            model.SheraP = SheraP
    ...
    libtrisolv.delay_line(
        model.Ybuffer_pointer,
        model.Zrp_pointer,
        model.Zrp1_pointer,
        model.Zrp2_pointer,
        model.Zrp3_pointer,
        model.Dev_pointer,
        model.YZweig_pointer,
        ctypes.c_int(model.YbufferLgt),
    ...
    libtrisolv.solve_tridiagonal(
        ctypes.byref(model.tridata), model.r_pointer, model.Qpointer, ctypes.c_int(n)
    )
    zero_val = model.RK4_0 * model.Qsol[0] + model.RK4G_0 * (
        model.g[0] + model.p0x * F0
    )
...
class cochlea_model:
    ...
    def __init__(self):
        ...
        self.stiffness_coupler = 1.0 / float(2.28e-11)
        self.p0 = float(2e-5)
    ...
    def init_model(
        ...
        probe_freq,
        sheraPo,
        compression_slope=0.4,
        Zweig_irregularities=1,
        non_linearity_type="vel",
        KneeVar=1.0,
        low_freq_irregularities=1,
        subject=1,
        IrrPct=0.05,
    ):
        ...
        self.SheraPo = (
            self.SheraPo + sheraPo
        )  # can be vector or single value, line changed so it can work with both single and vector
        ...
        self.Rth_norm = 10 ** (self.Rth / 20.0 / self.KneeVar)
        lf_limit = self.ctr
        if self.use_Zweig == 0:
            lf_limit = 0
        ...
        Theta0 = np.arctan(
            ((self.PoleE - self.PoleS) * factor) / ((self.RthV2 / self.RthV1) - 1.0)
        )
        Theta = Theta0 / 2.0
        Sfoc = (self.PoleS * factor) / (self.RthV2 / self.RthV1)
        Se = np.cos((np.pi - Theta0) * 0.5)
        self.Sb = Sfoc * Se
        self.Sa = Sfoc * np.sqrt(1.0 - (Se**2))
    ...
    def initCochlea(self):
        ...
        self.ZweigMso = 2.0 * self.rho / (self.bm_width * self.scalaHeight)
        self.ZweigL = 1.0 / (2.3030 * self.Greenwood_alpha)
        ...
        self.dx = self.bm_length / (1.0 * self.n)
    ...
    def initMiddleEar(self):
        ...
        self.p0x = self.ZweigMso * self.dx / (1.0 * self.ZweigMpo * self.bm_width)
    ...
    def SetDampingAndStiffness(self):
        self.f_resonance = (
            self.Greenwood_A * 10 ** (-self.Greenwood_alpha * self.x) - self.Greenwood_B
        )
        self.ctr = np.argmin(np.abs(self.f_resonance - 100.0))
        if self.low_freq_irregularities:
            self.ctr = self.n + 1
        self.onek = np.argmin(np.abs(self.f_resonance - 1000.0))
        self.omega = 2.0 * np.pi * self.f_resonance
    ...
    def initZweig(self):
        ...
        self.YbufferLgt = int(
            np.amax(self.delay)
        )  # quick fix, maybe can be shorter but it works so better not touch
    ...
    def initGaussianElimination(self):
        n = self.n + 1
        self.ZweigMs = (
            self.ZweigMso * self.ZweigOmega_co
        ) / self.omega  # TAPERING self.omega*
        ...
        self.ZAH[1 : n - 1] = (
            -self.ZweigMs[0 : n - 2] * self.omega[1 : n - 1] / self.omega[2:n]
        )
        #        self.ZAH[0] = -1.
    ...
    def SheraParameters(self):  # same as in fortran
        a = (
            self.SheraP + np.sqrt((self.SheraP**2.0) + self.c * (1.0 - self.SheraP**2))
        ) / self.c
        self.SheraD = 2.0 * (self.SheraP - a)
        self.SheraMu = 1.0 / (2.0 * np.pi * a)
        self.SheraRho = (
            2.0
            * a
            * np.sqrt(1.0 - (self.SheraD / 2.0) ** 2.0)
            * np.exp(-self.SheraP / a)
        )

    def ZweigImpedance(self):
        n = self.n + 1
        MudelayExact = (2 * np.pi) * self.SheraMu / (self.omega * self.dt)
        Mudelay = np.floor(MudelayExact) + 1.0
    ...
    def compression_slope_param(self, slope):
        ...
        vst1dB = (
            20 * np.log10(v1) + K1dB
        )  # velocity with the two poles when the compression starts
    ...
    def polecalculation(self):  # TODO
        ...
        if self.non_linearity == 1:  # To check
            ...
            Theta0 = np.arctan(
                ((self.PoleE - self.PoleS) / ((Yknee2F / Yknee1F) - 1.0))
            )
            Theta = Theta0 / 2.0
            ...
            Se = sin_Theta
            Sb = Sfoc * Se
            Sa = Sfoc * np.sqrt(1.0 - (1.0 * (Se**2)))
            Sxp = (Yvect - 1.0) * cos_Theta / cos_Theta0
        ...
        while j < length:
            ...
            if self.probe_freq == "all":
                ...
                self.Vsolution[j, :] = self.V1[
                    self.probe_points
                ]  # storing the decided probe points
                self.Ysolution[j, :] = self.Y1[self.probe_points]
