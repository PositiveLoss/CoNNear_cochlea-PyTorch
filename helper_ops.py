...
def slice_1dsignal(
    signal, window_size, winshift, minlength, left_context=256, right_context=256
):
    """
    ...
    for beg_i in range(0, n_samples, winshift):
        ...
        if beg_i_context < 0:
            slice_ = np.concatenate(
                (np.zeros((1, left_context - beg_i)), np.array([signal[:end_i]])),
                axis=1,
            )
            ...
            slice_ = np.concatenate(
                (np.array([signal[beg_i_context:]]), np.zeros((1, end_i - n_samples))),
                axis=1,
            )
        # print(slice_.shape)
